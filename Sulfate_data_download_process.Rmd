---
title: "Data Downloading and Processing Workflow v6"
author: "Liyang Qin"
date: "2024-11-07"
...
system.time(for (analyte in analyte.name){
 ...
 #Try 5 times for downloading to prevent internet issue; in reality, it may need more try to download all the data   
    for(i in 1:5){
      try({
        cat("this is", i, "try\n\n")
...
## Step 2: Combining 5 analytes spreadsheets together
```{r}
download_states <- fread('states_conus.csv')
par_name <-  "sulfate"

#combining 5 sulfate analytes spreadsheets together from multiple CONUS
...
## Step 3: Check if NA is present in several columns in the imported dataset
```{r}
...
## Step 4: Read in the list of sites downloaded from WQP website
```{r}
...
## Step 5: Data Subsetting by selecting columns we need
```{r}
columns_select <- c(
  'site_no',
  'station_nm',
...
# #For those has measurevalues but does not have unit, assign the detection limit unit
# value.no.unit <- so4.dectlim_subset[!is.na(ResultMeasureValue) & is.na(ResultMeasure.MeasureUnitCode)]
...
#Conversion
if (par_name == "sulfate"){
  so4.unit[ResultMeasure.MeasureUnitCode == "mg/l", value := ResultMeasureValue * 1e-3/m_so4]
  so4.unit[ResultMeasure.MeasureUnitCode == "mg/L", value := ResultMeasureValue * 1e-3/m_so4]
  so4.unit[ResultMeasure.MeasureUnitCode == "ug/l", value := ResultMeasureValue * 1e-6/m_so4]
  so4.unit[ResultMeasure.MeasureUnitCode == "ug/L", value := ResultMeasureValue * 1e-6/m_so4]
  so4.unit[ResultMeasure.MeasureUnitCode == "ueq/L", value := ResultMeasureValue / 2e6]
  so4.unit[ResultMeasure.MeasureUnitCode == "ueq/l", value := ResultMeasureValue / 2e6]
  so4.unit[ResultMeasure.MeasureUnitCode == "ppm", value := ResultMeasureValue / (1e3 * m_so4)]
  so4.unit[ResultMeasure.MeasureUnitCode == "umol/L", value := ResultMeasureValue / 1e6]
}
...
# Test block: check the difference between Total and Dissolved in samplefractiontext, find if the difference: if large, then we need to reject the previous assumption that we can treat total and dissolved as the same and vice versa.
```{r}
#  so each single day might have different ResultSampleFractionText: dissolved or total, but based on the result below suggested, the dissolved and total are close to each other, we can treat them as the same.
...
(dt_frac <- test.sample.fraction[, .N, by = .(site_no, ActivityStartDate)][order(-N)])

result <- if (2 %in% unique(dt_frac$N)) {
    # merge data
    dt_merged <- dt_frac[N == 2][test.sample.fraction, 
        on = c("site_no", "ActivityStartDate")]
    
    # Check hydrologic events
    event_counts <- dt_merged[, .(event_count = uniqueN(HydrologicEvent)), 
        by = .(site_no, ActivityStartDate)]
    
    # Filter out the data that has two events
    dt_filtered <- dt_merged[event_counts[event_count == 2, 
        .(site_no, ActivityStartDate)], 
        on = .(site_no, ActivityStartDate)]
    
    # Calculate the difference between total and dissolved
    result_diff <- dt_filtered[, .(diff = value[ResultCategory == "Total"] - 
        value[ResultCategory == "Dissolved"]), 
        by = .(site_no, ActivityStartDate)]
    
    # Check the quantile of the difference
    quantile(result_diff$diff, na.rm = TRUE)
}

## fix. plot the quantile plot
ggplot(result_diff) + 
...
## Step 13: Remove duplicate based on the sample category and check total-dissolved difference, and average for single day
```{r}
...
#The assumption is that dissolved and total are the near the same, this block also test if the difference between the two are large or small: if small, then we can stay with the same assumption; if not, then we need to change to the original ResultSampleFrationText and groupby that.
...
```{r}
# we can mannually adjust this parameter to fit our needs, here we chose 0.1% as it rules out extremly high and low values. Shuang here used 99.5%, we think too many was being removed (apprx. 20K values incl. both upper and lower quantiles). Now it removes less than 4k.
...
## Step 16: Save the final dataset as RDS file
```{r}
